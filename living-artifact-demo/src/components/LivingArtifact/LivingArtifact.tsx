...
export function LivingArtifact({
  ...
  const coreMaterialRef = useRef<InstanceType<typeof ArtifactMaterialImpl>>(null);
  const glowMaterialRef = useRef<InstanceType<typeof GlowMaterialImpl>>(null);
    ...
    <group>
      <mesh ref={glowRef}>
        <primitive
          ...
          attach="geometry"
        />
        <glowMaterial ref={glowMaterialRef} attach="material" />
      </mesh>
      <mesh ref={coreRef}>
        <primitive object={getGeometry(2, radius)} attach="geometry" />
        <artifactMaterial ref={coreMaterialRef} attach="material" />
      </mesh>
      <pointLight
        ref={lightRef}
